From Stdlib Require Import FunctionalExtensionality PropExtensionality Classical
...
Lemma remove_subset{X:Type}(S:Setof X)(x :X):
subset (remove S x) S.
Proof.
now intros y (Hmy & _).
Qed.

Lemma remove_not_member{A:Type}(S:Setof A)(x:A): ~member (remove S x) x.
Proof.
now intros (Hm & Hd).
Qed.
...
- 
  destruct Hm as (x & Hm & Heq);subst.
  unfold comp.
  now do 2 apply member_fmap.
-
  destruct Hm as (y & Hm & Heq); subst.
  destruct Hm as (x & Hm' & Heq); subst.
  now exists x.
Qed.         

Lemma fmap_cst{A B : Type}(S:Setof A)(c:B):
~is_empty S ->
fmap S (fun _:A => c) = single c.
Proof.
intro Hns.
apply set_equal; split; intro Hm.
-
...
-
 rewrite member_single_iff in Hm;subst.
 rewrite not_empty_member in Hns.
 destruct Hns as (x & Hmx).
 now exists x.
Qed. 


Definition cprod{A B : Type}(S1 : Setof A)(S2 : Setof B):=
  fun c => member  S1 (fst c) /\ member S2 (snd c).

Lemma fmap_cprod_fst{A B : Type}(S1 : Setof A)(S2 : Setof B):
~is_empty S2 ->
fmap (cprod S1 S2) fst = S1.
Proof.
intro Hne.
apply set_equal; split; intro Hmx.
-
 now destruct Hmx as ((u,v) &  (Hmu & Hmv) & Heq);subst.
-
rewrite not_empty_member in Hne.
destruct Hne as (u & Hmu).
now exists (x,u).
Qed.


Lemma fmap_cprod_snd{A B : Type}(S1 : Setof A)(S2 : Setof B):
~is_empty S1 ->
fmap (cprod S1 S2) snd = S2.
Proof.
intro Hne.
apply set_equal; split; intro Hmx.
-
...
Lemma fmap_cprod_single_right{A B C:Type}(f: A * B -> C)
(S:Setof A) (b:B):
fmap (cprod S (single b)) f =
fmap S (fun a : A => f (a, b)).
Proof.
apply set_equal; split ; intro Hmx.
-
  destruct Hmx as ((u,v) & (Hmu,Hmv) & Heq); subst;
  cbn in *.
...
Lemma fmap_cprod_single_left{A B C:Type}(f: A * B -> C)
(S:Setof B) (a:A):
fmap (cprod (single a) S) f =
fmap S (fun b : B => f (a, b)).
Proof.
apply set_equal; split ; intro Hmx.
-
  destruct Hmx as ((u,v) & (Hmu,Hmv) & Heq); subst;
